...
1.  默认的 entry './src/index.js'
2.  默认的 output './dist/main.js'
3.  cli 指定 mode， wepback --mode development(npm 包为 wepback-cli)，即可使用一个`webpack.config.js`文件，而不是两个了
...
1.  `module.rules`给处理`require.ensure`或`import`的各个文件，贯穿整个构建过程，只要是碰到匹配的文件
...
1.  在此文件 body 结尾处(默认)，加上 script[src='bundle.js']，并输出 html 文件到`dist`目录，（相对于 dist 目录的位置是相对于 src 的？）
2.  可以在 ejs 文件里写模板，
...
exports.plugins = [new CleanWebpackPlugin(["dist"])];
```
